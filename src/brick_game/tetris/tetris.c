...
GameInfo_t updateCurrentState() {
  ...
  for (int i = 0; i < NEXT_SIZE; i++, y++) {
    for (int j = 0; j < NEXT_SIZE; j++, x++) {
...
void userInput(UserAction_t action, bool hold) {
  ...
  if (action == Terminate) game->state = EXIT_STATE;
...
void reset_field(GameData *game) {
  ...
  game->tetris.next = (int **)calloc(NEXT_SIZE, sizeof(int *));
  for (int i = 0; i < NEXT_SIZE; i++) {
    game->tetris.next[i] = (int *)calloc(NEXT_SIZE, sizeof(int));
  }
}

/**
 * Clear the GameData fields, filling it with zeroes.
 */
